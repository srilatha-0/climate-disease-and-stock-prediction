...
# =========================
# PATHS (keep same as your original)
# =========================
...
# =========================
# STEP 2 — PROCESS STOCK DATA (WEEKLY)
# =========================
...
for file in os.listdir(STOCK_PATH):
    ...
    # Create weekly columns
    df["Year"] = df["Date"].dt.year
    df["Week"] = df["Date"].dt.isocalendar().week

    weekly = df.groupby(["Year", "Week"])["Close"].mean().reset_index()
    weekly["Return"] = weekly["Close"].pct_change().fillna(0)

    stock_name = file.replace(".csv", "")
    stock_weekly[stock_name] = weekly
...
# =========================
# STEP 3 — STATE ANALYSIS + MERGE (WEEKLY)
# =========================
all_state_files = []
...
for idx, file_path in enumerate(all_state_files, 1):
    try:
        ...
        # ANALYSIS
        # =========================
        for stock_name, sdf in stock_weekly.items():
            ...
            for year in ["2017","2018","2019","2020","2021"]:
                ...
                # convert month spikes → approximate weeks (4 weeks per month)
                weeks = []
                for m in months:
                    m = int(float(m))
                    weeks += list(range((m-1)*4+1, (m-1)*4+5))

                for w in weeks:
                    val = sdf[(sdf["Year"] == int(year)) & (sdf["Week"] == w)]
                    if not val.empty:
        ...
        if len(selected_stocks) > 0 and dengue_col in df.columns:
            weekly_dengue = df.groupby(["Year","Week"])[dengue_col].mean().reset_index()
            for stock in selected_stocks:
                stock_close_col = f"{stock}_Close"
                stock_data = df.groupby(["Year","Week"])[stock_close_col].mean().reset_index()
                merged_temp = pd.merge(weekly_dengue, stock_data, on=["Year","Week"])
...
print("\n🎉 DONE — FULL WEEKLY PIPELINE + CORRELATION COMPLETE!")