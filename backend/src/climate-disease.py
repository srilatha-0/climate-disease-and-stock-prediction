...
# ✅ FIX 1: Added missing comma after "data"
...
for idx, file_path in enumerate(tqdm(all_files, desc="Processing states")):
    try:
        ...
        # ✅ FIX 2: Remove correct dengue column (no data leakage)
        feature_cols = [c for c in df.columns if c not in ["Data", "year", dengue_col]]
...
# =========================
# ✅ FIX 3: Correct state encoding
# =========================
le = LabelEncoder()

train_X["state_enc"] = le.fit_transform(train_X["state"])
test_X["state_enc"] = le.transform(test_X["state"])
